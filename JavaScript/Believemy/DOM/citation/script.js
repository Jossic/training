...
let nou = document.querySelector("#nouveau");
let dernier = citations[0];
let nbAleatoire = 0;

nou.addEventListener("click", () => {
  do {
    nbAleatoire = Math.round(Math.random() * (citations.length - 1) + 0);
  } while (dernier == nbAleatoire);
  cit.textContent = citations[nbAleatoire][0];
  aut.textContent = citations[nbAleatoire][1];
  dernier = nbAleatoire;
});
